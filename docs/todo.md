...
- [ ] observability. tracing is imported but no structured metrics, no request latency histograms, no error rate counters. no health endpoint beyond admin API. operators cannot tell if it is healthy or slowly dying. kovarex #8 (blocked on #1-7)
