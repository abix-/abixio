...
- [ ] mc client throughput gap. root cause found and fixed: StreamingBlob::wrap() used s3s StreamWrapper which returns RemainingLength::unknown(), causing hyper to use chunked transfer encoding. replaced with SizedStream that reports exact remaining_length so hyper uses Content-Length. also fixed buffered GET path (get_object_buffered) which had the same issue. needs re-benchmark with mc to confirm fix closes the gap
